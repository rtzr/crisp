...
do {
    if a.count >= 6 && a[1] == "enhance", let mode = parseMode(a[4]), let q = FileQuality(rawValue: a[5]) {
        ...
        die("usage:\n  filetool <in> <out> <wav|m4a> [ll]\n  filetool batch <in> <outdir> <wav|m4a> <csv-levels>\n  filetool enhance <in> <out> <off|noise|voice|clean> <fast|hq> [natural|warm|bright] [podcast|meeting|none]", 2)
    }
} catch {
